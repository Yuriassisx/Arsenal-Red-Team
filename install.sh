...
# ---- 2) copia os arquivos (TUDO menos tests/, __pycache__, .pyc, venv, .git) ----
info "copiando arquivos para $PREFIX (excluindo tests/)..."
$S mkdir -p "$PREFIX"
EXCLUDES=(--exclude=tests --exclude=__pycache__ --exclude='*.pyc' --exclude=.git \
          --exclude=venv --exclude='.venv' --exclude='*.log' --exclude='install.sh')
if command -v rsync >/dev/null; then
  $S rsync -a "${EXCLUDES[@]}" "$SRC_DIR"/ "$PREFIX"/
else
  # fallback sem rsync: tar filtrando os excludes
  ( cd "$SRC_DIR" && tar --exclude=tests --exclude=__pycache__ --exclude='*.pyc' \
      --exclude=.git --exclude=venv --exclude='*.log' -cf - . ) | ( $S tar -xf - -C "$PREFIX" )
fi
ok "arquivos copiados (tests/ ficou de fora)"
